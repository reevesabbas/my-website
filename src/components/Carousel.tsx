...
const PROJECTS = [
  {
    name: 'Ergo',
    title: 'Task App Api',
    iconBtn: { icon: VscGithub, link: 'https://github.com/Ergo-Tasks/ergo-api' },
  ...
  },
  {
    ...
    iconBtn: { icon: VscGithub, link: 'https://github.com/reevesabbas/my-website' },
  ...
  },
  {
    ...
    iconBtn: { icon: VscGithub, link: 'https://github.com/reevesabbas/journal' },
  ...
  return (
    <div className="mt-6 w-full h-fit select-none relative dark:bg-gray bg-lightestPurple rounded-xl drop-shadow-lg">
      <div className="absolute align-top w-full flex justify-between top-1/2 items-center opacity-100">
        ...
        >
          {' '}
          <IoIosArrowDropleftCircle size={32} fill={`${theme === 'dark' ? '#FE5E19' : '#006FDC'}`} />{' '}
        </button>
        <button
        ...
        >
          {' '}
          <IoIosArrowDroprightCircle size={32} fill={`${theme === 'dark' ? '#FE5E19' : '#006FDC'}`} />{' '}
        </button>
      </div>
      ...
      >
        ...
        <div className="flex flex-row md:justify-end justify-center space-x-4 w-full">
          <a href={iconBtn.link} target="_blank" rel="noopener noreferrer">
            {' '}
            <IconButton Icon={iconBtn.icon} size={30} />{' '}
          </a>
          <h3> {title} </h3>
        </div>
        <p> {body} </p>
      ...
      <div className="flex absolute justify-center mx-auto flex-row space-x-1.5 w-full h-4 pt-3">
        {PROJECTS.map((el, index) => (
          <div
            className={`w-2 h-2 rounded-full dark:bg-orange bg-blue ${
              currIndex === index ? 'opacity-100' : 'opacity-30'
            }`}
